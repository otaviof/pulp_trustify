...
@pytest.fixture()
def npm_repository(pulp_api, wait_for_task):
    repo_url = f"{pulp_api.api_url}/repositories/npm/npm/"
    repo_resp = pulp_api.post(
        repo_url,
        json={"name": "test-npm-repo"},
    )
    repo_resp.raise_for_status()
    repo_href = repo_resp.json()["pulp_href"]

    guard_url = f"{pulp_api.api_url}/contentguards/trustify/guard/"
    guard_resp = pulp_api.post(
        guard_url,
        json={"name": "test-npm-guard"},
    )
    guard_resp.raise_for_status()
    guard_href = guard_resp.json()["pulp_href"]

    distro_url = f"{pulp_api.api_url}/distributions/npm/npm/"
    distro_resp = pulp_api.post(
        distro_url,
        json={
            "name": "test-npm-distro",
            "base_path": "test-npm",
            "repository": repo_href,
            "content_guard": guard_href,
        },
    ...
    distro_list = pulp_api.get(
        distro_url,
        params={"name": "test-npm-distro"},
    )
    distros = distro_list.json().get("results", [])
    base_path = distros[0]["base_path"] if distros else "test-npm"
...
@pytest.fixture()
def uploaded_npm_vulnerable(
    npm_repository,
    pulp_api,
    upload_npm_package,
    wait_for_task,
):
    repo_href, _ = npm_repository
    tarball = "tests/e2e/fixtures/is-svg-4.2.1.tgz"
    task_href = upload_npm_package(repo_href, tarball, "is-svg", "4.2.1")
    data = wait_for_task(task_href)
    content_href = next(
        (r for r in data.get("created_resources", []) if "/content/" in r),
        None,
